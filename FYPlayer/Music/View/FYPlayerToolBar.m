...
@implementation FYPlayerToolBar
...
- (void)upadte
{
    ...
    if (self.isPlaying && self.isDragging == NO) {
        //1.更新进度条
        double currentTime = [FYMusicTool sharedFYMusicTool].player.currentTime;
...
{
    return [[[NSBundle mainBundle] loadNibNamed:@"FYPlayerToolBar" owner:nil options:nil]lastObject];
}
...
- (void)setPlayingMusic:(FYMusic *)playingMusic{
    ...
    //歌曲总时间
    double duration = [FYMusicTool sharedFYMusicTool].player.duration;
...
- (IBAction)sliderChange:(UISlider *)sender {
    //1.播放器的进度
    [FYMusicTool sharedFYMusicTool].player.currentTime = sender.value;
    //2.工具条的时间
